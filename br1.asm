 ...
 TITLE AL-051 : An AL Program that lets the user type in an algebric
              ; expression, ending with a carriage return, that contains round
              ; (parenthesis), square, and curly brackets. As the expression
              ; is being typed in, the program evaluates each character. If at
              ; any point the expression is incorrectly bracketed ( too many
              ; right brackets or a mismatch between left and right brackets),
              ; the program tells the user to start over. After the carriage
              ; return is typed, if the expression is correct, the program
              ; displays "expression is correct". If not, the programdisplays
              ; "too many left brackets". In both cases, the program ask the
              ; user if he or she wants to continue. If the user types 'Y',
              ; the program runs again. Your program does not need to store
